# MODULE ADAPTED FROM https://github.com/nhutter/lkf_tools
# REFERENCE: https://tc.copernicus.org/articles/13/627/2019/
import numpy as np
...
def detect_segments(lkf_thin,eps_thres=0.1):
    ...
    while num_nodetect > 0:
        ...
        if (deactivate_segs_muln.size > 0):
            ...
            try:
                new_starts_deact_ind = np.rollaxis(
                    np.array(
                        np.where(
                            (num_neighbours[neigh_deactivate[:,0].astype('int'), neigh_deactivate[:,1].astype('int')]<=2) &
                            (nodetect[neigh_deactivate[:,0].astype('int'), neigh_deactivate[:,1].astype('int')]==1))
                    ),1).squeeze()
